...
public class GraphValidTree {
    ...
    //build graph, then bfs/dfs traverse graph, find if any nodes visited twice, also we will detect if they are all connected
    //Time:O(N + E) = O(N), E is roughly equal to N,
    // initialising graph with N, iterating over and inserting E edges.
    //Each node is added to the data structure once. This means that the outer loop will run N times. For each of the N nodes,
    // its adjacent edges is iterated over once. In total, this means that all E edges are iterated over once by the inner loop.
    // This, therefore, gives a total time complexity of O(N+E)
    //SPACE:O(N + E) = O(N), E is roughly equal to N.
    ...
    public boolean validTreeBFS(int n, int[][] edges) {
        ...
        //detect cycle
        Set<Integer> visited = new HashSet<Integer>();//mark visited nodes, if we meet a visited node, return false
        Deque<Integer> queue = new ArrayDeque<>();
        queue.offer(0);
        while(!queue.isEmpty()) {
            int curNode = queue.poll();
            if (!visited.add(curNode)) return false;//visited, cycle detected
